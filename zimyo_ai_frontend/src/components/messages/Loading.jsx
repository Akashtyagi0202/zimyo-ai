...
const SKELETONS = {
  ...
  table: () => (
    <div className="space-y-2">
      <div className="flex gap-4 pb-2 border-b border-gray-100">
        {[1, 2, 3].map(i => <div key={i} className="h-2.5 bg-gray-200 rounded flex-1 animate-pulse" />)}
      </div>
      {[1, 2, 3, 4].map(i => (
        <div key={i} className="flex gap-4 py-1.5">
          {[1, 2, 3].map(j => <div key={j} className="h-2.5 bg-gray-100 rounded flex-1 animate-pulse" />)}
        </div>
      ))}
    </div>
  ),
    ...
    <div className="space-y-3">
      {[1, 2, 3].map(i => (
        <div key={i} className="flex items-center gap-2.5">
          ...
          <div className="h-2.5 bg-gray-100 rounded flex-1 animate-pulse" />
        </div>
      ))}
    </div>
  ),
  ...
  generic: () => (
    <div className="space-y-2.5">
      ...
      <div className="h-2.5 bg-gray-100 rounded w-5/6 animate-pulse" />
...
export default function Loading({ msg }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 w-full max-w-md mt-2 animate-fade-in">
      {message && (
        <p className="text-xs text-gray-500 mb-3 flex items-center gap-2">
          <span className="w-1.5 h-1.5 bg-zimyo-500 rounded-full animate-pulse" />
          {message}
        </p>
      )}
      <Skeleton />
